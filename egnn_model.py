...
class E_GCL(nn.Module):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d=0, act_fn=nn.SiLU(), residual=True,
        ...
        input_edge = input_nf * 2 # Times 2 since the edge requires both feature vectors from the two nodes connected by the edge
        ...
        # Corresponds to \phi_e in the message update of the E(n) paper
        ...
        # Corresponds to \phi_h for the feature vector update
        ...
        # Corresponds to \phi_x for the coordinate vector update (node embeddings)
        coord_mlp = []
    ...
    """
    This method computes the messages 
        m_{ij} = \phi_e(source, target, radial, edge_attr) = \phi_e(h_i, h_j, r_{ij}, a_{ij})
    """
    def edge_model(self, source, target, radial, edge_attr):
        ...
        out = self.edge_mlp(out) # Apply MLP to the inputs
        if self.attention:
    ...
    """
    This method computes the feature vector update
        h_i^{l+1} = \phi_h(h_i^l, m_i) where m_i = \sum_{j\in N(i)}m_{ij}
    """
    ...
    """
    This method computes the coordinate vector (node embedding) update
    ...
    """
    Computes the 'radial' parameter corresponding to the relative distances between two nodes based on coordinate embeddings
    """
